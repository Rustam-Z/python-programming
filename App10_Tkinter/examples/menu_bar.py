...
def command_new():
    ...
    Label(root, text="You Clicked New ...").pack()


def command_cut():
    Label(root, text="Awesome, it is working!").pack()


def question():
    response = messagebox.askquestion("Question", "Do you liked Menu Bars?")
    if response == "yes":
        Label(root, text="Ugh!!!").pack()
    else:
        Label(root, text="Sorry ...").pack()


def command_copy():
    Button(root, text="Popup", command=question).pack()
